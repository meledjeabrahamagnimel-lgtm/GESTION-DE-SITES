...
class WebPushTest extends TestCase
{
    ...
    public function test_la_cloche_enregistre_l_abonnement_de_l_appareil(): void
    {
        $serveur = CleEc::genererPaire();
        config(['webpush.cle_publique' => CleEc::base64UrlEncoder($serveur['publique'])]);
        config(['webpush.cle_privee' => CleEc::base64UrlEncoder($serveur['privee'])]);

        $utilisateur = User::create([
            'name' => 'Testeur',
            'email' => 'appareil@exemple.test',
            'password' => 'mot-de-passe-de-test',
            'est_actif' => true,
        ]);
        ...
        $endpoint = 'https://fcm.googleapis.com/fcm/send/jeton-appareil';

        \Livewire\Volt\Volt::test('cloche-notifications')
            ->call('enregistrerAbonnement', $endpoint, 'cle-p256dh', 'cle-auth', 'Chrome sur Android')
            ->assertHasNoErrors();
        ...
        // Rejouer le même abonnement ne doit pas créer de doublon.
        \Livewire\Volt\Volt::test('cloche-notifications')
            ->call('enregistrerAbonnement', $endpoint, 'cle-p256dh-2', 'cle-auth-2', 'Chrome sur Android');
