<?php

use Modules\Noyau\Entreprises\Modeles\Entreprise;
use function Livewire\Volt\{state, mount, computed};

state(['entrepriseId']);

mount(function (Entreprise $entreprise) {
    $this->entrepriseId = $entreprise->id;
});

$entreprise = computed(fn () => Entreprise::with(['sites.responsable', 'sites.ville'])->findOrFail($this->entrepriseId));

$repartition = computed(function () {
    return $this->entreprise->sites->map(function ($site) {
        $responsableId = $site->responsable_id ?? $site->ville->responsable_id;

        return [
            'site' => $site,
            'responsables' => \App\Models\User::where('entreprise_id', $this->entrepriseId)->whereHas('roles', fn ($q) => $q->where('name', 'responsable_site'))->where('id', $responsableId)->count(),
            'commerciaux' => \Modules\Noyau\Exploitation\Modeles\Commercial::where('ville_id', $site->ville_id)->where('est_spontane', false)->count(),
        ];
    });
});

?>

<x-a-venir titre="{{ $this->entreprise->nom }}" description="Répartition des personnes par catégorie et par site.">
        <div class="tableau-conteneur">
            <table class="tableau">
                <thead>
                    <tr>
                        <th>Site</th>
                        <th>Responsables</th>
                        <th>Commerciaux</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($this->repartition as $ligne)
                        <tr style="border-bottom:1px solid var(--th-ligne,#E2E0D8);">
                            <td style="font-weight:600;">
                                <span style="display:inline-block; width:8px; height:8px; border-radius:99px; background:{{ $ligne['site']->ville->couleur }}; margin-right:6px;"></span>
                                {{ $ligne['site']->nom }}
                            </td>
                            <td>{{ $ligne['responsables'] }}</td>
                            <td>{{ $ligne['commerciaux'] }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </x-a-venir>
